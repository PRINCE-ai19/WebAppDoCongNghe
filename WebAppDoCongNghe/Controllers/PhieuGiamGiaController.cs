    ...
    public class PhieuGiamGiaController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            return Ok(
                ...
                {
                    Success = true,
                    Message = "l?y danh sách phi?u gi?m giá thành công",
                    Data = data
                }
                );
        }
        ...
        public async Task<IActionResult> Create(PhieuGiamGiaRequest request)
        {
            ...
            return Ok(
                ...
                {
                    Success = true,
                    message = "t?o thành công phi?u gi?m giá",
                    data = PhieuGiam
        ...
        public async Task<IActionResult> Update(int id, [FromBody] PhieuGiamGiaRequest request)
        {
            ...
            if (entity == null) {
                return BadRequest(new ApiRespone { 
                 Success = false,
                 Message ="l?i không tìm th?y phi?u gi?m giá"
                });
            }
           ...
           .FirstOrDefaultAsync(x => x.Id == resquest.PhieuGiamGiaID);
            ...
            if (voucher == null)
                return BadRequest(new {
                 sucsess = false,
                 message ="Phi?u gi?m giá không t?n t?i"
                });

            if (DateTime.Now < voucher.NgayBatDau || DateTime.Now > voucher.NgayKetThuc)
                return BadRequest (new { 
                sussess = false,
                message ="phi?u này dã h?t h?n s? d?ng"
                });

            if (voucher.SoLuong <= 0)
                return BadRequest(new { sussess = false , message ="phi?u dã h?t s? lu?ng" });
            ...
            if(userVoucher != null)
            {
                return BadRequest(new {
                 success = false,
                 message ="B?n dã nh?n phi?u này r?i"
                });
            }
            ...
            {
                success = true,
                message = "b?n nh?n phi?u gi?m giá thành công"
            });
